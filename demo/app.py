"""
Interactive CLI Audit Demo for CompGraphRAG.
Demonstrates compliance determination, graph path traversal, rule evaluation, and conformal confidence set routing.
"""
...
class CompGraphRAGDemoApp:
    def __init__(self, dataset_path: str):
        ...
        self.evaluator = CompGraphRAGEvaluator(dataset_path=dataset_path)
    ...
    def run_demo_query(self, query_id: str) -> Dict[str, Any]:
        ...
        # Un-leaked candidate graph path retrieval matching evaluation harness
        retrieved_path, hybrid_score = self.evaluator._retrieve_top_path(target["question"], target["hop_count"])

        # Step 1: Neuro-symbolic Rule Engine Evaluation over retrieved path
        ...
        subgraph_fmt = self.subgraph_extractor.format_subgraph_json(retrieved_path)
        nl_walk = self.subgraph_extractor.generate_natural_language_walk(retrieved_path)
        ...
        print("--------------------------------------------------------------------------------")
        print(f"FINAL SYSTEM DETERMINATION: {suggested}")
        ...
        return {
            ...
            "gold_determination": target["gold_determination"],
